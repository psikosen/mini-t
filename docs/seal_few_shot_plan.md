...
5. **Reporting & observability** – Emit structured logs (matching the Sherlock protocol schema) from the new training/evaluation paths so CI can surface unexpected regressions when few-shot adapters are toggled. Implemented across the math, training, and evaluation harnesses so CORE/SEAL runs surface metrics in Sherlock-compliant form. 【F:nanochat/scripts/seal_fewshot_math.py†L33-L70】【F:nanochat/scripts/seal_fewshot_train.py†L63-L238】【F:nanochat/scripts/base_eval.py†L18-L220】
